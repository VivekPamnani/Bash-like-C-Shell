...
void pipe_setup(char **args, int *num_tokens, int begin_loc)
{
    ...
    if(fork() == 0)
    {
        ...
        args[delim_loc - 1] = NULL;
        execvp(args[begin_loc], &args[begin_loc]);
    }
    else
    {
...
void pipe_loop(int *pip_0, int *pip_1, char **arg, int *num_tok, int beg_loc, int num_chld) //IMPORTANT: invert arguments while passing when recursion.
{
    ...
    if(fork() == 0)
    {
        ...
        // execvp;
        arg[del_loc - 1] = NULL;
        execvp(arg[beg_loc], &arg[beg_loc]);
    }
    else
    {
   ...
   for(int i = 0; i < 100; i++)                                             //allocating memory
    ...
    { 
        ...
        for(int i = 1; i <= tokens; i++)
        {
            ...
            else if(!strcmp(args[i][1], "jobs"))
            {
                jobs();
            }
            else if(!strcmp(args[i][1], "kjob"))
            {
                kjob(atoi(args[i][2]), atoi(args[i][3]));
            }
            else if(!strcmp(args[i][1], "kjob"))
            {
                bg(atoi(args[i][2]));
            }
            else if(!strcmp(args[i][1], "overkill"))
            {
                overkill();
            }
