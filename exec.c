...
void jobs()
{
    ...
    {
        printf("[%d] %s %s [%d]\n", i, children[i]>0?"Running":"Stopped", children_name[i], children[i]>0?children[i]:-children[i]);
    }
}

void kjob(int jobid, int sign)
{
    if(jobid < children_index)
        kill(children[jobid], sign);
    else
